                                           ...
                                           std::int32_t P)
{
    ...
    for (std::int32_t current_tile = 0; current_tile < (N + kBlockSize - 1) / kBlockSize; ++current_tile)
    {
        ...
        __shared__ std::int32_t shared_A[kBlockSize][kBlockSize];
        __shared__ std::int32_t shared_B[kBlockSize][kBlockSize];

        // Load A and B into shared memory
        if (row < M && (current_tile * blockDim.x + threadIdx.x < N))
        {
        ...
        if (column < P && (current_tile * blockDim.y + threadIdx.y < N))
        {
            shared_B[threadIdx.y][threadIdx.x] = B[(current_tile * blockDim.y + threadIdx.y) * P + column];
        }
        else
        {
