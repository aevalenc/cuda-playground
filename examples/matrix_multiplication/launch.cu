...
void PrintMatrix(const std::int32_t* A, const std::int32_t N)
{
    std::cout << "[";
...
double LaunchGPU(std::int32_t N)
{
    ...
    std::int32_t* host_B = new std::int32_t[N * N];
    std::int32_t* host_C = new std::int32_t[N * N];
    ...
    for (std::int32_t i = 0; i < N; ++i)
    {
        for (std::int32_t j = 0; j < N; ++j)
        {
            ...
            host_B[j + N * i] = 2 * (i + j);  // Example initialization, can be random or specific values
        }
    ...
    if (cudaMalloc((void**)&device_A, sizeof(std::int32_t) * N * N) != cudaSuccess)
    {
        std::cerr << "Failed to allocate device memory for device matrix A\n";
        return -1;
    }
    if (cudaMalloc((void**)&device_B, sizeof(std::int32_t) * N * N) != cudaSuccess)
    {
        std::cerr << "Failed to allocate device memory for device matrix B;\n";
        cudaFree(device_A);
        return -1;
    }
    if (cudaMalloc((void**)&device_C, sizeof(std::int32_t) * N * N) != cudaSuccess)
    {
    ...
    if (cudaMemcpy(device_A, host_A, sizeof(std::int32_t) * N * N, cudaMemcpyHostToDevice) != cudaSuccess)
    {
    ...
    if (cudaMemcpy(device_B, host_B, sizeof(std::int32_t) * N * N, cudaMemcpyHostToDevice) != cudaSuccess)
    {
